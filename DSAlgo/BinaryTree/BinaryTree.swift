...
//insert at the end
func insert(node : Node? , insertValue : Int) -> Node {
    if node?.value == nil {
...
func findSuccessor(node : Node?, succesorOf : Int) -> Int {
    if node?.value == nil {
        return 0
    }
...
func depthOrderTraversal(node : Node?) {
...
func isFullTree(node : Node?) -> Bool {
    if node == nil {
        return true
    }
...
func rangeSumBSTC(node : Node? , L : Int , R : Int) -> Int {
    var result = 0
    if node != nil {
...
func DST(node : Node?, searchElement : Int) -> Bool  {
    guard let node = node else {
        return false
    }
...
var result = 0
func sumOfLeftLeaves(_ root: Node?) -> Int {
...
func isSymetric( _ node : Node?) -> Bool {
    //send node and clone of the node to check for mirror
    return isMirror(node,node)
}

func isMirror(_ t1 : Node? , _ t2 : Node?) -> Bool {
    if t1 == nil && t2 == nil {
        return true
    }
    if t1 == nil || t2 == nil {
...
func sortedArrayToBst(_ nums : [Int]) -> Node? {
    
    return BST(nums, 0 , nums.count-1)
    
}

private func BST(_ nums : [Int], _ left : Int , _ right : Int) -> Node? {
    if left > right {
        return nil
    }
    let mid = left + (right-left) / 2
    //add mid value as root
    let root = Node(value : nums[mid],leftChild: nil,rightChild: nil)
